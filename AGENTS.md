...
## 技術スタック（確定）
TypeScript / Next.js 16（App Router）/ OpenAI（テキスト=`gpt-5.4-mini`・画像=`gpt-image-2`・将来の議事録文字起こし=`gpt-realtime-whisper`）/ ホスティング Vercel。
...
- **AI 呼び出し**: `lib/ai.ts` に集約。モデル名は `.env.local`（`OPENAI_TEXT_MODEL` / `OPENAI_IMAGE_MODEL`）で変更可。将来の議事録→用語抽出に備えた `extractTermsFromText` の土台あり。

...
## 技術スタック（確定）
TypeScript / Next.js 16（App Router）/ OpenAI（テキスト=`gpt-5.4-nano`・画像=`gpt-image-2`（品質 `medium` 既定）・議事録の文字起こし=`gpt-4o-mini-transcribe`）/ ホスティング Vercel。テキストは安い nano（料金最優先）。画像は「用語を絵で説明する」役割なので、図解がくっきり見える `medium` を既定にしている（プロンプトも“説明的な図解”を指示。実装は `lib/ai.ts`）。
**データベース・認証は未確定（保留中）** — ユーザーの確認待ち。

## アーキテクチャ上の差し替えポイント（DB・認証が未確定のため）
- **データ保存**: `lib/store/`（用語=`TermRepository`＋`json-store.ts`→`.data/terms.json`／議事録=`MinutesRepository`＋`json-minutes-store.ts`→`.data/minutes.json`）。当面はローカルJSON実装。DB が決まったら同インターフェースの実装を作り `lib/store/index.ts` で差し替える。画面側は変更不要。
- **認証**: `lib/auth.ts`（名前＋合言葉をクッキーに入れるだけの仮実装。署名なしなので強い認証ではない）。本認証が決まったら差し替える。
- **AI 呼び出し**: `lib/ai.ts` に集約。モデル名・画質は `.env.local`（`OPENAI_TEXT_MODEL` / `OPENAI_IMAGE_MODEL` / `OPENAI_IMAGE_QUALITY` / `OPENAI_TRANSCRIBE_MODEL`）で変更可（既定値は `lib/config.ts`）。議事録は、リアルタイム文字起こし（`createRealtimeTranscriptionSecret` で一時トークン発行→ブラウザが WebRTC で OpenAI Realtime に直接接続。鍵はサーバー側のまま）か、音声ファイルからの一括文字起こし（`transcribeAudio`）→`extractTermsFromText` で用語抽出（画面は `/minutes`）。

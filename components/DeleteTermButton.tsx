...
// 削除ボタン（F-07）。押すと「本当に削除しますか？」の確認を画面の中に出し、
...
//   「キャンセル(false)」を返す。そのため以前は確認ダイアログが出ないまま、
//   フォーム送信もキャンセルされ「削除できない」状態になっていた。
...
export function DeleteTermButton({ id, word }: { id: string; word: string }) {
  ...
  return (
    <form action={deleteTermAction} className="flex flex-wrap items-center gap-2">
      <input type="hidden" name="id" value={id} />
      {confirming ? (
        <>
          <span className="text-sm text-muted-foreground">
            「{word}」を削除しますか？元に戻せません。
          </span>
          <ConfirmDeleteSubmit />
          <button
            type="button"
